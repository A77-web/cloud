...
namespace cloud
{
  ...
  {
    private:
      std::string _back_dir="./backup_dir/";//备份文件的实际储存路径
      std::string _pack_dir="./pack_dir/";//压缩包储存路径
      std::string _download_prefix="/download/";//下载链接前缀路径 http://192.168.109.130/download/a.txt
      std::string _pack_subfix=".zip";//压缩包后缀名
      std::string _back_info_file="./backup.dat";//储存备份信息的文件
      //用url作为key是因为当前端下载文件时，就会发送过来url
      std::unordered_map<std::string,FileInfo> _back_info;//<url,fileinfo>备份信息
    public:
      //数据的增删改查
      //增：输入一个文件名，在接口中获取信息，生成压缩包名称，填充结构体，压入hash表
      bool Insert(const std::string &pathname)
      {
        ...
        info.real_path=pathname;//实际储存路径 ./backup_dir/a.txt
        info.file_size=FileUtil(pathname).Size();//文件大小
        info.back_time=FileUtil(pathname).Mtime();//最后一次修改时间（备份时间）
        info.pack_flag=false;//刚上传的文件都是非压缩状态
        info.pack_path=_pack_dir+info.filename+_pack_subfix;//压缩包文件名/pack_dir/a.txt.zip
        _back_info[info.url_path]=info;//以url为key，info为value添加到map中；
      ...
      //查：查询所有备份信息（前端展示界面需要文件名-下载链接-大小-备份时间）
      //    查询单个信息（文件下载，获取文件实际备份路径）
      ...
      //通过实际路径获取备份信息
      bool SelectOneByRealpath(const std::string &url_path,FileInfo *info)
      {
        for(auto it =_back_info.begin();it!=_back_info.end();++it)
        {
          if(it->second.real_path==url_path)
          {
            *info=it->second;
            return true;
          }

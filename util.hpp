...
namespace cloud
{
  class FileUtil
  {
    ...
    bool Read(std::string *body)
    {
      ...
      body->resize(fsize);
      ifs.read(&(*body)[0],fsize);//string::c_str() 返回值const char*
      if(ifs.good()==false)
      {
    ...
    bool ScanDirectory(std::vector<std::string> *arry)
    {
      ...
      for(auto &a :  fs::directory_iterator(_name))
      {
        ...
        //当前我的目录遍历中，只能获取普通文件信息，针对目录不做深度处理
        //std::string pathname=fs::path(a).filename().string();是纯文件名
    ...
    bool Compress(const std::string &packname)
    {
      ...
      {
        std::cout<<"compress pack data failed!\n";
        return false;
      }
